...
from mai.db.session import get_session, session_scope
...
class TaskReporter:
    def __init__(
        self, task_id: int, queue: "TaskQueue", min_interval: float = 0.5
    ) -> None:
...
def _handle_organize_rollback(ctx: TaskContext) -> dict[str, Any]:
    ...
    with session_scope() as session:
        summary = rollback_manifest(
            session, manifest_id, settings, progress_cb=progress_cb
        )
...
class TaskQueue:
    ...
    def _process_task(self, task_id: int) -> None:
        ...
        try:
            context = TaskContext(
                task_id=task_id, payload=payload or {}, reporter=reporter
            )
